...
class Fingerprint(JenkinsBase):
    ...
    def __init__(self, baseurl, id, jenkins_obj):
        ...
        assert self.RE_MD5.search(id), "{} does not look like a valid id".format(id)
        url =  "{}/fingerprint/{}/".format(baseurl, id)
        super().__init__(self, url, poll=False)
        self.id = id
    ...
    def validate_for_build(self, filename, job, build):
        ...
        if self._data["fileName"] != filename:
            log.info("Filename from jenkins ({}) did not match provided ({})".format(self._data["fileName"], filename))
            return False
            ...
            if usage_item["name"] == job:
                for range in usage_item["ranges"]["ranges"]:
                    if range["start"] <= build <= range["end"]:
                        log.info("This artifact was generated by {} between build {} and {}".format(job, range["start"],  range["end"]))
                        return True
        return False
    ...
    def validate(self):
        try:
            ...
            raise ArtifactBroken("Artifact {} seems to be broken, check {}".format(self.id, self.baseurl))
        except urllib.error.HTTPError:
            raise ArtifactBroken("Unable to validate artifact id {} using {}".format(self.id, self.baseurl))
        return True
